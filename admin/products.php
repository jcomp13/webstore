<?php
 ...
 $saved_image='';
     ...
     if(isset($_GET['edit'])){
		...
		if (isset($_GET['delete_image'])){
			...
			$image_url = '/teststore/images/products/'. $images[$imgi]; 
			unlink($image_url);          //  jjc file not being removed from hosted directory
			unset($images[$imgi]);
 ...
 if ($_POST){
	 ...
	 $sizeString = sanitize($sizeString);     //   Should have the trailing comma removed
	 ...
	 foreach($required as $field){
		 if ($_POST[$field] == ''){
			 $errors[] = 'All Fields and Asterisk are required';
			 break;
		 } else {
...
//	     	$photo=$_FILES['photo'];
		     ...
		     $dbpath .= '/teststore/images/products/'.$uploadName;
		     if ($mimeType != 'image') {
	 ...
	 if (!empty($errors)){
		 ...
		 $insertSql = "insert into products (title, price, list_price, brand, categories, sizes, description, image) 
		 values ('$title', '$price', '$list_price', '$brand', '$category', '$sizeString', '$description', '$dbpath')";
		 if (isset($_GET['edit'])){
...
<div class="modal fade" id="sizesModal" tabindex="-1" role="dialog" aria-labelledby="sizesModalLabel" aria-hidden="true">
 ...
 <table class="table table-bordered table-condensed table-striped">
	...
	//	echo 'childid = ' . $childID . '<br>';    6  -  5    from products-> categories
	...
	?>
	    <tr>
		    <td>
			   <a href="products.php?edit=<?=$product['id'];?>"  class="btn btn-xs btn-default"><span class="glyphicon glyphicon-pencil"></span></s>
			   <a href="products.php?delete=<?=$product['id'];?>"  class="btn btn-xs btn-default"><span class="glyphicon glyphicon-remove"></span></s>			   
			</td>
		    <td><?=$product['title'];?></td>			
